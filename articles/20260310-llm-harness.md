...
実装した関数を LLM が「ツール」として認識できるようにするには、ツール定義のリストを渡す必要があります。
...
:::message
OpenAI の API では、サーバー側で履歴の保持をサポートする Responses API も提供されています。これを利用すれば毎回すべての履歴を送る必要はなく、キャッシュ効率も向上します。

ただし、Codex CLI では履歴を最適化しながら送ることが想定されているらしく、専用のエンドポイントでは履歴の保持がサポートされていません。👉[詳細](https://github.com/7shi/codex-oauth)
:::
...
実装に見られるように、過去の履歴を毎回送信することで LLM はコンテキストを認識しています。LLM がその都度学習して内容を記憶しているわけではありません。この点を踏まえて、LLM の「記憶」と「推論」の関係について解説した記事です。
