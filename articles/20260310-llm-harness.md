...
まず [Ollama](https://ollama.com) をインストールし、モデルを `pull` しておきます。モデルは以下の4つから選択することを推奨します。GPU がなくても動作しますが、モデルサイズの 1.5 倍程度のメモリが必要になります。
...
このような制御のための枠組みは「ハーネス」と呼ばれます。ハーネスは本来、馬具や幼児用の安全帯などを指し、対象を目的の方向に正しく制御するための道具です。
...
LLM は本来、テキストを生成することしかできません。ユーザーが「調べたいこと」や「直したいこと」を相談すると、LLM が必要なコマンドを提案し、ユーザーがそのコマンドを実行して結果を貼り付ける必要があります。LLM はその結果を分析して、ようやく回答を返せます。
...
ツールの実装においては、まず「LLM から呼び出すための関数」を Python 側で定義します。そして、それらの関数の名前・目的・引数の情報をツールリストとして LLM に渡すことで、LLM は「どのツールを、どんな引数で実行すべきか」を判断し、ツールの実行依頼を返せるようになります。

### コマンド実行ツール

シェル（bash）のコマンドを実行するツールを実装します。`ls` でファイル一覧を見たり、`grep` で検索したりと、これだけで LLM が外部の環境を操作するための基本動作をカバーできます。
...
基本となるファイルの読み取りツールを実装します。LLM がファイルの内容を確認したいときに呼び出されます。ファイルの内容が多すぎる場合は、行数を制限して返すこともできます。
...
ファイルの作成や上書きを行うツールを実装します。LLM が新しいファイルを作成したり、既存のファイル全体を更新したりするために呼び出されます。
...
`s02_tool_use.py` は、「ユーザーとの対話」と「ツールの実行」がネストしたループとして構成されています。
...
:::message
フローの構成上、ツールコールの後に必ずテキストの回答で締め括ることになります。
:::
...
モデルは `<think>` タグでの（空の）思考の直後、`<tool_call>` タグを出力します。システムプロンプトで示したフォーマット指示とツール定義だけで、モデルは学習時に見たとおりのタグ付きテキストを生成しています。
