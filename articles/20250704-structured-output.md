...
**期待されるスコア:**
-   **実験1（指示なし）**: 評価基準が与えられていないため、モデルは文章の内容自体を評価することが期待されます。評価対象の文章は一般的な論説文であるため、**平均スコア4.0以上**を期待値とします。
...
-   **o3 (実験3: 1.60, 実験5: 2.20)**
    ...
    - **分析**: 「ない」と明確に答えられる度合いをスコアに反映させる自己解釈ロジックが見られます。
...
これらの方法は各モデルが解釈に迷う余地を減らし、幻覚や自己解釈を抑制する上で同程度に有効でした。メンテナンス性やプロンプトの簡潔さを考慮すると、プロンプトでキーと指示を対応付ける「キー対応方式」（実験5）が依然としてバランスの取れた良い選択肢ですが、モデルによってはフィールド名で直接指示する方が安定する場合もありました。
...
| GPT-4.1 | 比較的常識的だが、プロンプトが曖昧な場合、独自の基準を優先する傾向がある。コストと相談。 |
| GPT-4.1 mini | 独自の基準を優先する傾向がある。コストパフォーマンス良好。 |
| o3 | 独自の基準を優先する傾向があるため、発想の自由度よりも指示への準拠が求められる評価用途には不向き。高コスト。 |
