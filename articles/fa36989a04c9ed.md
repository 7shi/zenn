...
一方、**`format='json'`と`think=True`を併用した場合**、`thinking`フィールドは`null`になります。
...
```json
{
  "message": {
    "thinking": null,
    "content": "{\"reasoning\": \"思考プロセス...\", \"answer\": \"東京\"}"
  }
}
```
...
リクエストにおける`format`パラメータの型は、[ollama/_types.py:154](https://github.com/ollama/ollama-python/blob/63ca74762284100b2f0ad207bc00fa3d32720fbd/ollama/_types.py#L154)で以下のように定義されています。
