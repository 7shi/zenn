...
ARM NEON の `vqtbl1q_s8` は 16 バイトのテーブルを 1 命令で引けるため、2 要素単位の 9 通り（16 エントリに収まる）が自然に対応します。一方 x86 の `_mm256_shuffle_epi8` は 256-bit 幅で並列化するため、3 要素単位を主軸にして 16 エントリのテーブルに収める構成が効率的です。K 次元の端数には 2 要素版を併用します。
...
MAD は実行時にテンソルのサイズを動的に処理するため、コンパイル済みバイナリはどのモデルでも利用できます。一方 LUT は行列サイズ（M, K）をコンパイル時定数としてハードコーディングするため、**モデルごとに専用バイナリをビルドする必要があります**。対応していないサイズが渡されると計算がスキップされ、不正な出力になります。
...
| 形状依存性 | 比較的低い | モデルごとにビルドが必要 |
| 実装の複雑さ | 低い | 高い |
| 3 値性の活用 | 2-bit 圧縮によるメモリ削減 | 乗算回避 |

プロンプト評価では MAD が LUT の約 2.4〜2.6 倍高速です。MAD の Weight/Activation Parallel 最適化が効果的に機能しているためだと考えられます。一方、1 トークンずつ生成するフェーズでは両者はほぼ同等の速度です。ベンチマークの詳細は以下の記事を参照してください。

https://qiita.com/7shi/items/24eb2b2b9e6149cb1d54

プロンプト評価で圧倒的に有利であることや、モデルごとのビルドが不要であることを踏まえると、実用上は MAD だけを使えば十分ではないでしょうか。
...
Falcon 3 というモデルを TL2 に対応させる作業を行ってみて、モデルごとにビルドが必要なのはかなり面倒だと感じました。

https://zenn.dev/7shi/scraps/60580be831d272
